...
import { useStatementStats } from "@/hooks/useApi";
import { LoadingSpinner } from "@/components/LoadingStates";

const Statistics = () => {
    const { data: statsData, loading, error } = useStatementStats("month");

    if (loading) {
        return (
            <div className="card max-md:overflow-hidden">
                <LoadingSpinner message="Loading statement statistics..." />
            </div>
        );
    }
        ...
        return (
            <div className="card max-md:overflow-hidden">
                <div className="p-5 text-center text-red-500">
                    Error loading statement statistics: {error}
                </div>
            </div>
        );
    }

    const statistics = (statsData as any)?.data || [];
